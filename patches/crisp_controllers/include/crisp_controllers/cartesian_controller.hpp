...
private:
  ...
  /** @brief Subscription for the async rollout control-loop heartbeat */
  rclcpp::Subscription<std_msgs::msg::UInt64>::SharedPtr async_inference_heartbeat_sub_;
  /** @brief Transient-local publisher acknowledging heartbeats consumed by the realtime loop */
  rclcpp::Publisher<std_msgs::msg::UInt64>::SharedPtr async_inference_watchdog_ack_pub_;
  std::shared_ptr<realtime_tools::RealtimePublisher<std_msgs::msg::UInt64>>
    realtime_async_inference_watchdog_ack_pub_;
  /** @brief Transient-local publisher for the latched async watchdog status */
  rclcpp::Publisher<std_msgs::msg::Bool>::SharedPtr async_inference_watchdog_status_pub_;
  ...
  /** @brief Publish the latched async inference watchdog status. */
  void publish_async_inference_watchdog_status(bool timed_out);

  /** @brief Publish a heartbeat acknowledgement from the realtime update loop. */
  void publish_async_inference_watchdog_ack(std::uint64_t heartbeat_value);
  ...
  realtime_tools::RealtimeBuffer<std::shared_ptr<geometry_msgs::msg::PoseStamped>>
    target_pose_buffer_;
  std::atomic<std::uint64_t> target_pose_sequence_{0};
  std::uint64_t observed_target_pose_sequence_{0};
  TargetCommandWatchdog async_inference_watchdog_;
  std::mutex async_inference_heartbeat_authority_mutex_;
  std::array<std::uint8_t, RMW_GID_STORAGE_SIZE> async_inference_heartbeat_publisher_gid_{};
  bool async_inference_heartbeat_publisher_gid_latched_{false};
  LatestTargetCommandAcknowledgement async_inference_watchdog_ack_;
  bool async_inference_watchdog_status_{false};
  bool async_inference_watchdog_status_publish_pending_{false};
