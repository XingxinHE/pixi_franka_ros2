...
std::uint64_t steady_time_nanoseconds() noexcept {
...
bool is_valid_target_pose_message(const geometry_msgs::msg::PoseStamped & message) noexcept {
  return is_valid_target_pose(
...
CartesianController::update(const rclcpp::Time & time, const rclcpp::Duration & /*period*/) {
  ...
  const TargetCommandWatchdogUpdate async_watchdog_update = async_inference_watchdog_.update(
    steady_time_nanoseconds(),
    params_.async_inference_watchdog.enabled,
    seconds_to_nanoseconds(params_.async_inference_watchdog.timeout_seconds));
  const bool async_command_authorized = !params_.async_inference_watchdog.enabled ||
    (async_watchdog_update.armed && !async_watchdog_update.timed_out);
  const bool auxiliary_targets_allowed =
    auxiliary_target_commands_allowed(params_.async_inference_watchdog.enabled);

  if (
    params_.async_inference_watchdog.enabled && async_watchdog_update.has_new_command &&
    async_command_authorized) {
    publish_async_inference_watchdog_ack(async_watchdog_update.accepted_command_value);
  }

  // Check if new targets are available. Discard latched pose replays while the async rollout lease
  // is absent or stale; otherwise CRISP's timer could move without a live control loop.
  const std::uint64_t target_pose_sequence = target_pose_sequence_.load(std::memory_order_acquire);
  if (target_pose_sequence != observed_target_pose_sequence_) {
    observed_target_pose_sequence_ = target_pose_sequence;
    if (async_command_authorized) {
      parse_target_pose_();
    }
  }
  if (new_target_joint_) {
    if (async_command_authorized && auxiliary_targets_allowed) {
      parse_target_joint_();
    }
  ...
  if (async_watchdog_update.event == TargetCommandWatchdogEvent::kTimedOut) {
    ...
    RCLCPP_ERROR(
      get_node()->get_logger(),
      "Async inference heartbeat timed out after %.3f s; holding the measured Cartesian pose.",
      params_.async_inference_watchdog.timeout_seconds);
    publish_async_inference_watchdog_status(true);
  }

  if (params_.async_inference_watchdog.enabled) {
    target_wrench_.setZero();
  }

  if (params_.async_inference_watchdog.enabled && !async_command_authorized) {
    q_target = q;
    q_ref = q;
    dq_ref.setZero();
  }
  ...
  log_debug_info(time);
  if (async_inference_watchdog_ack_.pending()) {
    publish_async_inference_watchdog_ack(async_inference_watchdog_ack_.value());
  }
  if (async_inference_watchdog_status_publish_pending_) {
    publish_async_inference_watchdog_status(async_inference_watchdog_status_);
  }
...
CartesianController::on_configure(const rclcpp_lifecycle::State & /*previous_state*/) {
  ...
  observed_target_pose_sequence_ = target_pose_sequence_.load(std::memory_order_acquire);
  async_inference_watchdog_.reset_ignoring_existing_commands();
  async_inference_watchdog_ack_.reset();
  {
    std::lock_guard<std::mutex> lock(async_inference_heartbeat_authority_mutex_);
    async_inference_heartbeat_publisher_gid_.fill(0);
    async_inference_heartbeat_publisher_gid_latched_ = false;
  }
  new_target_joint_ = false;
  new_target_wrench_ = false;
  new_target_stiffness_ = false;
  ...
  auto target_pose_callback =
    ...
    if (!is_valid_target_pose_message(*msg)) {
      RCLCPP_WARN_THROTTLE(
        get_node()->get_logger(),
        *get_node()->get_clock(),
        1000,
        "Ignoring invalid target_pose message: translation and quaternion must be finite, and "
        "the quaternion must have unit norm.");
      return;
    }
    ...
    target_pose_sequence_.fetch_add(1, std::memory_order_release);
  };
  ...
  auto async_inference_heartbeat_callback = [this](
                                              const std::shared_ptr<std_msgs::msg::UInt64> msg,
                                              const rclcpp::MessageInfo & message_info) -> void {
    const std::string & heartbeat_topic = params_.async_inference_watchdog.heartbeat_topic;
    const auto publisher_infos = get_node()->get_publishers_info_by_topic(heartbeat_topic);
    if (publisher_infos.size() != 1) {
      RCLCPP_WARN_THROTTLE(
        get_node()->get_logger(),
        *get_node()->get_clock(),
        1000,
        "Ignoring async inference heartbeat: topic '%s' has %zu publishers, expected exactly 1.",
        heartbeat_topic.c_str(),
        publisher_infos.size());
      return;
    }

    std::array<std::uint8_t, RMW_GID_STORAGE_SIZE> publisher_gid;
    const auto & rmw_message_info = message_info.get_rmw_message_info();
    std::copy_n(rmw_message_info.publisher_gid.data, publisher_gid.size(), publisher_gid.begin());
    if (publisher_gid != publisher_infos.front().endpoint_gid()) {
      RCLCPP_WARN_THROTTLE(
        get_node()->get_logger(),
        *get_node()->get_clock(),
        1000,
        "Ignoring queued async inference heartbeat whose publisher is no longer the sole "
        "discovered endpoint.");
      return;
    }

    bool publisher_matches_authority = true;
    {
      std::lock_guard<std::mutex> lock(async_inference_heartbeat_authority_mutex_);
      if (!async_inference_heartbeat_publisher_gid_latched_) {
        async_inference_heartbeat_publisher_gid_ = publisher_gid;
        async_inference_heartbeat_publisher_gid_latched_ = true;
      } else {
        publisher_matches_authority = publisher_gid == async_inference_heartbeat_publisher_gid_;
      }
    }
    if (!publisher_matches_authority) {
      RCLCPP_WARN_THROTTLE(
        get_node()->get_logger(),
        *get_node()->get_clock(),
        1000,
        "Ignoring async inference heartbeat from a publisher that does not own the active lease.");
      return;
    }

    async_inference_watchdog_.note_command(msg->data, steady_time_nanoseconds());
  };

  async_inference_heartbeat_sub_ = get_node()->create_subscription<std_msgs::msg::UInt64>(
    params_.async_inference_watchdog.heartbeat_topic,
    rclcpp::QoS(1),
    async_inference_heartbeat_callback);

  const auto watchdog_event_qos = rclcpp::QoS(1).reliable().transient_local();
  async_inference_watchdog_ack_pub_ = get_node()->create_publisher<std_msgs::msg::UInt64>(
    params_.async_inference_watchdog.ack_topic, watchdog_event_qos);
  realtime_async_inference_watchdog_ack_pub_ =
    std::make_shared<realtime_tools::RealtimePublisher<std_msgs::msg::UInt64>>(
      async_inference_watchdog_ack_pub_);

  async_inference_watchdog_status_pub_ = get_node()->create_publisher<std_msgs::msg::Bool>(
    params_.async_inference_watchdog.status_topic, watchdog_event_qos);
  realtime_async_inference_watchdog_status_pub_ =
    std::make_shared<realtime_tools::RealtimePublisher<std_msgs::msg::Bool>>(
      async_inference_watchdog_status_pub_);
  publish_async_inference_watchdog_status(false);
  ...
  if (auxiliary_targets_allowed) {
    ...
    stiffness_sub_.reset();
    RCLCPP_INFO(
      get_node()->get_logger(),
      "Async inference pose-only contract enabled: target_joint, target_wrench, and "
      "target_stiffness subscriptions are disabled.");
  }
  ...
  RCLCPP_INFO(
    get_node()->get_logger(),
    "Async inference heartbeat topic: %s",
    params_.async_inference_watchdog.heartbeat_topic.c_str());
  RCLCPP_INFO(
    get_node()->get_logger(),
    "Async inference watchdog acknowledgement topic: %s",
    params_.async_inference_watchdog.ack_topic.c_str());
  RCLCPP_INFO(
    get_node()->get_logger(),
    "Async inference watchdog status topic: %s",
    params_.async_inference_watchdog.status_topic.c_str());
...
void CartesianController::publish_async_inference_watchdog_ack(std::uint64_t heartbeat_value) {
  async_inference_watchdog_ack_.request(heartbeat_value);
  if (!realtime_async_inference_watchdog_ack_pub_) {
    return;
  }

  std_msgs::msg::UInt64 ack_message;
  ack_message.data = async_inference_watchdog_ack_.value();
#if REALTIME_TOOLS_NEW_API
  if (realtime_async_inference_watchdog_ack_pub_->try_publish(ack_message)) {
    async_inference_watchdog_ack_.mark_published();
  }
#else
  if (realtime_async_inference_watchdog_ack_pub_->trylock()) {
    realtime_async_inference_watchdog_ack_pub_->msg_ = ack_message;
    realtime_async_inference_watchdog_ack_pub_->unlockAndPublish();
    async_inference_watchdog_ack_.mark_published();
  }
#endif
}

void CartesianController::publish_async_inference_watchdog_status(bool timed_out) {
  async_inference_watchdog_status_ = timed_out;
  async_inference_watchdog_status_publish_pending_ = true;
  if (!realtime_async_inference_watchdog_status_pub_) {
    return;
  }

  std_msgs::msg::Bool status_message;
  status_message.data = timed_out;
#if REALTIME_TOOLS_NEW_API
  if (realtime_async_inference_watchdog_status_pub_->try_publish(status_message)) {
    async_inference_watchdog_status_publish_pending_ = false;
  }
#else
  if (realtime_async_inference_watchdog_status_pub_->trylock()) {
    realtime_async_inference_watchdog_status_pub_->msg_ = status_message;
    realtime_async_inference_watchdog_status_pub_->unlockAndPublish();
    async_inference_watchdog_status_publish_pending_ = false;
  }
#endif
}
